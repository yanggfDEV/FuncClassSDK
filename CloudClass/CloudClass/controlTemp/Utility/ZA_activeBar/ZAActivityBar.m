...
#define  NOTICE_request_timeOut  @"NOTICE_request_timeOut"
...
@property (nonatomic, strong, readonly) NSTimer *fadeOutTimer_timeout;
...
@synthesize fadeOutTimer,fadeOutTimer_timeout, overlayWindow, barView, stringLabel, spinnerView, imageView, actionIndicatorView, actionIndicatorLabel;
@synthesize activityBarStadusForAction;
@synthesize boundTouch;
///////////////////////////////////////////////////////////////
...
/*   dfm 添加了延时自动取消方法    todo ：添加回调函数 */
+ (void) showWithStatus:(NSString *)status forAction:(NSString *)action  duration:(NSTimeInterval)duration{
    
    [[ZAActivityBar sharedView]showWithStatus:(NSString *)status forAction:(NSString *)action  duration:(NSTimeInterval)duration];
}

+ (void) showWithStatus:(NSString *)status forAction:(NSString *)action duration:(NSTimeInterval)duration boundTouch:(BOOL)YesOrNo
{
    [ZAActivityBar sharedView].boundTouch=YesOrNo;
    [[ZAActivityBar sharedView]showWithStatus:(NSString *)status forAction:(NSString *)action  duration:(NSTimeInterval)duration];
...
- (void) showWithStatus:(NSString *)status forAction:(NSString *)action  duration:(NSTimeInterval)duration  {
        ...
        [self.fadeOutTimer_timeout invalidate];
        dispatch_async(dispatch_get_main_queue(), ^{
            
        [self.activityBarStadusForAction  setObject:@"showing" forKey:action];
            
        self.fadeOutTimer_timeout = [NSTimer timerWithTimeInterval:duration target:self selector:@selector(dismissFromTimer_timeout:) userInfo:action repeats:NO];
        
        [[NSRunLoop currentRunLoop] addTimer:self.fadeOutTimer_timeout forMode:NSRunLoopCommonModes];
        if(!self.superview)
            [self.overlayWindow addSubview:self];
        // Add the action
        [self addAction:action withStatus:status];
...
+ (void) showErrorWithStatus:(NSString *)status forAction:(NSString *)action {
    [ZAActivityBar showErrorWithStatus:status duration:3.5f forAction:action];
}
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration forAction:(NSString *)action {
    ...
    [self.activityBarStadusForAction  setObject:@"ErroOrSuccess" forKey:action];
...
- (void)dismissFromTimer_timeout:(NSTimer *)timer{
    
    
    NSString *action = timer.userInfo;
    [ZAActivityBar dismissForAction:action];
    if ([[self.activityBarStadusForAction objectForKey:action ] isEqualToString:@"showing"])  {
        [ZAActivityBar  dismiss];
        [[NSNotificationCenter defaultCenter]  postNotificationName:NOTICE_request_timeOut object:self userInfo:nil ];
    }
...
- (void)setFadeOutTimer_timeout:(NSTimer *)newTimer{


    if(fadeOutTimer_timeout)
        [fadeOutTimer_timeout invalidate], fadeOutTimer_timeout = nil;
    
    if(newTimer)
        fadeOutTimer_timeout = newTimer;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if ((self = [super initWithFrame:frame])) {
        ...
        self.activityBarStadusForAction=[[NSMutableDictionary alloc ] init];
    }
...
- (UILabel *) actionIndicatorLabel {
    if (!actionIndicatorLabel) {
		...
		actionIndicatorLabel.font = [UIFont systemFontOfSize:12];
