...
static CGFloat s_backgroundScale = 1.f;
...
+ (void) showImageFrom:(UIImageView*)imageView  index:(int)index  imgUrlArray:(NSArray  *)imgArraay
{
    if (imageView.image) {
        ...
        [viewer showImageFrom:imageView index:index imgUrlArray:imgArraay];
    }
}
...
-(void)loadView {
    ...
    int  imgCont = (int)self.imgUrlArray.count-1;
    scrollView.contentSize = CGSizeMake(self.view.bounds.size.width*imgCont, self.view.bounds.size.height);
    ...
    imageView.center = CGPointMake(320*self.targetIndex+160, imageView.center.y);
    [self.zoomeableScrollView addSubview: imageView];
    
    for (int i=0; i<imgCont; i++) {
        UIImageView* imageView = [[UIImageView alloc] initWithFrame:self.view.bounds];
        imageView.contentMode = UIViewContentModeScaleAspectFit;
        imageView.clipsToBounds = YES;
        imageView.backgroundColor =[UIColor   blackColor];
        
        NSDictionary *d =[self.imgUrlArray   objectAtIndex:i];
        [imageView   sd_setImageWithURL:[NSURL  URLWithString:[d   objectForKey:@"big_photo"]] placeholderImage:[UIImage imageNamed:@"mainPageDefaultImg.png"]];
        
        UIScrollView* scrollView = [[UIScrollView alloc] initWithFrame:self.view.bounds];
        scrollView.center = CGPointMake(320*i+160, imageView.center.y);
        scrollView.maximumZoomScale = 10.f;
        scrollView.minimumZoomScale = 1.f;
        [scrollView  addSubview:imageView];
        scrollView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        [self.zoomeableScrollView addSubview: scrollView];
    }
    [self.zoomeableScrollView scrollRectToVisible:CGRectMake(320*self.targetIndex, 0,self.zoomeableScrollView.frame.size.width,self.zoomeableScrollView.frame.size.height) animated:NO];
...
- (void) onBackgroundTap {
    ...
    originalImageRect  = CGRectMake(self.originalImage.frame.origin.x, originalImageRect.origin.y, originalImageRect.size.width, originalImageRect.size.height);
...
- (void) adjustScrollInsetsToCenterImage {
    ...
    self.zoomeableScrollView.contentSize = CGSizeMake(self.view.bounds.size.width*10, self.view.bounds.size.height);
