...
+ (DNAlbumTableViewController *)shareStance
{
    static DNAlbumTableViewController *albumTableViewController = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        albumTableViewController = [[DNAlbumTableViewController alloc] init];
    });
...
- (void)setupData
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver: self selector:@selector(getselectImage:) name:Notice_Tag_selectImageArray object:nil];
}

- (void)setupView
{
    self.title = LOCALSTRING(@"albumTitle");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DNImageFlowViewController *imageFlowViewController = [[DNImageFlowViewController alloc] initWithGroupURL:url maxImageCount:7];
    [imageFlowViewController getSelectImageArrayMethodTwo:self.selectImageArray];
    [self.navigationController pushViewController:imageFlowViewController animated:YES];
    [tableView deselectRowAtIndexPath:indexPath animated:YES];
}

/**
 * @guangfu yang 15-12-23 14:52
 * 为了多相册选中图片，原来的数据不删除
 **/
- (void)getselectImage: (NSNotification *)notification {
    NSMutableArray *array = notification.object;
    for (ALAsset *alaSetStr in array) {
        if (![self.selectImageArray containsObject:alaSetStr]) {
            [self.selectImageArray addObject:alaSetStr];
        }
    }
}
