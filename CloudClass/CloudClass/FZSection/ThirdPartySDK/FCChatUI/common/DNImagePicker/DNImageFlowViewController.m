...
// 单次支持的最多张数
static NSUInteger const kDNImageFlowMaxSeletedNumber = 7;   // 默认值
#define Notice_Tag_selectImageArray @"Notice_Tag_selectImageArray"
//static NSUInteger const kDNImageFlowMaxSeletedNumber = 1;
...
@property (nonatomic, assign) NSUInteger imageFlowMaxSeletedNumber;
@end
...
- (instancetype)initWithGroupURL:(NSURL *)assetsGroupURL
{
    ...
    if (self) {
        ...
        self.imageFlowMaxSeletedNumber = kDNImageFlowMaxSeletedNumber;
    }
    return self;
}
...
{
    self = [self initWithGroupURL:assetsGroupURL];

    if (imageCount) {
        self.imageFlowMaxSeletedNumber = imageCount;
    } else {
        self.imageFlowMaxSeletedNumber = kDNImageFlowMaxSeletedNumber;
    }
    return self;
...
- (void)getSelectImageArrayMethodTwo: (NSMutableArray *)array {
    _selectedAssetsArray = [NSMutableArray new];
    [self.selectedAssetsArray addObjectsFromArray:array];
}
...
{    
    ...
    [self createBarButtonItemAtPosition:DNImagePickerNavigationBarPositionLeft
                      statusNormalImage:[UIImage imageNamed:@"nav_back"]
                   statusHighlightImage:[UIImage imageNamed:@"nav_back"]
    ...
    UIBarButtonItem *item1 = [[UIBarButtonItem alloc] initWithTitle:LOCALSTRING(@"preview") style:UIBarButtonItemStylePlain target:self action:@selector(previewAction)];
    [item1 setTintColor:[UIColor colorWithHexString:@"89ca30"]];
    item1.enabled = NO;
...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.selectedAssetsArray.count >= self.imageFlowMaxSeletedNumber) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:LOCALSTRING(@"alertTitle") message:[NSString stringWithFormat:LOCALSTRING(@"alertContent"),self.imageFlowMaxSeletedNumber] delegate:self cancelButtonTitle:LOCALSTRING(@"alertButton") otherButtonTitles:nil, nil];
        [alert show];
...
- (void)backButtonAction {
    [[NSNotificationCenter defaultCenter] postNotificationName:Notice_Tag_selectImageArray object:self.selectedAssetsArray];
