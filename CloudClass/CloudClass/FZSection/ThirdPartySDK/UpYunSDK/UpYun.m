 ...
 * begin
 **/
- (void)uploadFile:(id)file policy:(NSString *)policy signature:(NSString *)signature {
    if ([policy isEqualToString:@""] ||
        !policy ||
        [policy isKindOfClass:[NSNull class]] ||
        [signature isEqualToString:@""] ||
        !signature ||
        [signature isKindOfClass:[NSNull class]]) {
        return;
    }
    if([file isKindOfClass:[UIImage class]]){
        [self uploadImage:file policy:policy signature:signature];
    }else if([file isKindOfClass:[NSData class]]) {
        [self uploadFileData:file policy:policy signature:signature];
    }else if([file isKindOfClass:[NSString class]]) {
        [self uploadFilePath:file policy:policy signature:signature];
    }else {
        NSError *err = [NSError errorWithDomain:ERROR_DOMAIN
                                           code:-1999
                                       userInfo:@{@"message":@"传入参数类型错误"}];
...
- (void)uploadImage:(UIImage *)image policy:(NSString *)policy signature:(NSString *)signature {
    NSData *imageData = UIImagePNGRepresentation(image);
    [self uploadImageData:imageData policy:policy signature:signature];
}

- (void) uploadImageData:(NSData *)data policy:(NSString *)policy signature:(NSString *)signature {
    [self uploadFileData:data policy:policy signature:signature];
}

- (void)uploadFileData:(NSData *)data policy:(NSString *)policy signature:(NSString *)signature {
    AFHTTPRequestOperation * operation = [self creatOperationWithSaveKey:policy
                                                               signature:signature
                                                                    data:data
                                                                filePath:nil];
    [operation start];
...
- (void) uploadFilePath:(NSString *)path policy:(NSString *)policy signature:(NSString *)signature {
    AFHTTPRequestOperation * operation = [self creatOperationWithSaveKey:policy
                                                               signature:signature
                                                                    data:nil
                                                                filePath:path];
...
- (AFHTTPRequestOperation *)creatOperationWithSaveKey:(NSString *)policy
    ...
    NSString *policyStr = policy;
    NSString *signatureStr = signature;
    NSDictionary * parameDic = @{@"policy":policyStr, @"signature":signatureStr};
