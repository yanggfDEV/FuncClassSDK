...
- (id)init
{
    self = [super init];
    if (self) {
        self.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 250);//[UIScreen mainScreen].bounds.size.height

        delegate = self;
        useMotionEffects = false;
        buttonTitles = nil;//@[@"Close"];
...
- (IBAction)customIOS7dialogButtonTouchUpInside:(UIButton *)senders
{
    if (delegate != NULL) {
        [delegate customIOS7dialogButtonTouchUpInside:self clickedButtonAtIndex:[senders tag]];
    }

    if (onButtonTouchUpInside != NULL) {
        onButtonTouchUpInside(self, senders.tag);
    }
}
