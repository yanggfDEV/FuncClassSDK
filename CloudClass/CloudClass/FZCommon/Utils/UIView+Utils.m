...
- (void)addKeyboardCtrlView
{
    UIControl *ctrl = [[UIControl alloc] initWithFrame:self.bounds];
    ctrl.backgroundColor = [UIColor clearColor];
    ctrl.autoresizesSubviews = YES;
    [ctrl addTarget:self action:@selector(clearKeyboard)forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:ctrl];
    [self sendSubviewToBack:ctrl];
}
