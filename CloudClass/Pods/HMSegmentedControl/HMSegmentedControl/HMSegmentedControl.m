...
- (void)commonInit {
    ...
    self.shouldNotifyEveryTouch = NO;
    self.textImageBetweenSpace = 2;
...
- (void)drawRect:(CGRect)rect {
    ...
    //add by liuyong for imageLeft & imageRight
    else if (self.type == HMSegmentedControlTypeTextImageLeft){
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                imageXOffset = self.segmentWidth * idx + (self.segmentWidth - (textWidth + imageWidth + self.textImageBetweenSpace)) / 2;
...
//                textWidth = [self.segmentWidthsArray[idx] floatValue];
            ...
            textXOffset = imageXOffset + imageWidth + 2;
            CGRect imageRect = CGRectMake(imageXOffset, imageYOffset, imageWidth, imageHeight);
            CGRect textRect = CGRectMake(textXOffset, yOffset, textWidth, stringHeight);
    ...
    else if (self.type == HMSegmentedControlTypeTextImageRight){
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                //                imageXOffset = (self.segmentWidth * idx) + (self.segmentWidth / 2.0f) - (imageWidth / 2.0f);
                imageXOffset = (self.segmentWidth * idx);
                textXOffset = self.segmentWidth * idx + (self.segmentWidth - (textWidth + imageWidth + self.textImageBetweenSpace)) / 2;
//                textWidth = [self]//self.segmentWidth;
            } else if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                // When we are drawing dynamic widths, we need to loop the widths array to calculate the xOffset
                CGFloat xOffset = 0;
                NSInteger i = 0;
                ...
                imageXOffset = xOffset + ([self.segmentWidthsArray[idx] floatValue] / 2.0f) - (imageWidth / 2.0f); //(self.segmentWidth / 2.0f) - (imageWidth / 2.0f)
                textXOffset =  self.segmentEdgeInset.left + xOffset;
//                textWidth = [self.segmentWidthsArray[idx] floatValue];
            }
            
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f) - imageHeight / 2;
            imageXOffset = textXOffset + textWidth + 2;
            CGRect imageRect = CGRectMake(imageXOffset, imageYOffset, imageWidth, imageHeight);
            CGRect textRect = CGRectMake(textXOffset, yOffset, textWidth, stringHeight);
...
- (void)updateSegmentsRects {
    ...
    //add by liuyong
    else if ((self.type == HMSegmentedControlTypeTextImageLeft || self.type == HMSegmentedControlTypeTextImageRight) && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            ...
            self.segmentWidth = MAX(stringWidth + imageWidth + self.textImageBetweenSpace, self.segmentWidth);
        }];
    }else if ((self.type == HMSegmentedControlTypeTextImageLeft || self.type == HMSegmentedControlTypeTextImageRight) && HMSegmentedControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
...
//            CGFloat combinedWidth = MAX(imageWidth, stringWidth);
            CGFloat sumWidth = stringWidth + imageWidth + self.textImageBetweenSpace;
            
            [mutableSegmentWidths addObject:[NSNumber numberWithFloat:sumWidth]];
        }];
        self.segmentWidthsArray = [mutableSegmentWidths copy];
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        //Modified by liuyong
        if (self.shouldNotifyEveryTouch) {
